...
pub fn find_references(dex_name: &str, data: &[u8], query: &Query) -> Result<Vec<String>> {
    ...
    let hits = dex.scan_all_classes(kind, &target_set)?;
...
impl<'a> Dex<'a> {
    fn parse(data: &'a [u8]) -> Result<Self> {
        ...
        check_table(data, header.types_off, header.types_size, 4, "type_ids")?;
        check_table(data, header.fields_off, header.fields_size, 8, "field_ids")?;
        check_table(
            data,
            header.methods_off,
            header.methods_size,
    ...
    fn matching_strings(&self, pattern: &str) -> Result<Vec<u32>> {
        ...
        let encoded = encode_mutf8(pattern);
        let encoded_pattern = encoded
            .iter()
            .map(|byte| format!("\\x{byte:02x}"))
            .collect::<String>();
        let regex = Regex::new(&encoded_pattern)
            .with_context(|| format!("invalid search pattern {pattern:?}"))?;
        let mut out = Vec::new();
        for index in 0..self.header.strings_size {
            if regex.is_match(self.string_bytes(index)?) {
    ...
    fn matching_members(&self, query: &MemberQuery, methods: bool) -> Result<Vec<u32>> {
        ...
        let name_regex = query
            ...
            .transpose()
            .context("invalid member pattern")?;
        let class_regex = match &query.class {
            Some(ClassQuery::Fuzzy(pattern)) => {
                Some(Regex::new(&regex::escape(pattern)).context("invalid class pattern")?)
            }
            _ => None,
        };
        let mut out = Vec::new();
        for index in 0..size {
            ...
            let name_matches = match &name_regex {
                Some(regex) => regex.is_match(self.string_bytes(member.name_idx as usize)?),
                None => true,
            };
            if !name_matches {
                continue;
            ...
            let class_matches = match &query.class {
                ...
                Some(ClassQuery::Fuzzy(_)) => class_regex
                    .as_ref()
                    .is_some_and(|regex| regex.is_match(class_bytes)),
            };
            if class_matches {
                out.push(index as u32);
            }
    ...
    fn scan_all_classes(
        ...
        if self.header.classes_size < PARALLEL_SCAN_CLASSES {
            self.scan_all_classes_sequential(kind, target_set)
        } else {
            self.scan_all_classes_parallel(kind, target_set)
        }
    }
    ...
    fn scan_all_classes_sequential(
        ...
        for class_index in 0..self.header.classes_size {
            self.scan_class_index(class_index, kind, target_set, &mut hits)?;
        }
        Ok(hits)
    }
    ...
    fn scan_class_index(
        ...
        if class_data_off != 0 {
            self.scan_class_data(class_data_off, kind, target_set, hits)?;
        }
        Ok(())
    }
    ...
    fn scan_code(
        ...
        while pc + 2 <= code.len() {
            let opcode = code[pc];
            let units = instruction_units(code, pc)?;
            if units == 0 || pc + units * 2 > code.len() {
                bail!("invalid instruction width at code offset {}", start + pc);
            }
            if opcode_matches(kind, opcode) {
                let index = if kind == RefKind::String && opcode == 0x1b {
                    read_u32(code, pc + 2)?
                } else {
...
fn opcode_matches(kind: RefKind, opcode: u8) -> bool {
    match kind {
...
mod tests {
    ...
    fn parallel_and_sequential_scans_produce_identical_hits() {
        ...
        let (kind, targets) = dex
            ...
            .unwrap();
        let target_set: BTreeSet<u32> = targets.into_iter().collect();
        let sequential = dex.scan_all_classes_sequential(kind, &target_set).unwrap();
        let parallel = dex.scan_all_classes_parallel(kind, &target_set).unwrap();
    ...
    fn fuzzy_patterns_are_literal_substrings() {
        let regex = Regex::new(&regex::escape(".b[")).unwrap();
        assert!(regex.is_match(b"a.b[c]"));
        assert!(!Regex::new(&regex::escape("a.c")).unwrap().is_match(b"abc"));
    }
