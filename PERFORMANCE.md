...
### `classes` 的阶段构成与并行化（2026-09-12）

567,192 个类、58 MiB 输出；`classes --debug` 现在会打印阶段耗时（与其他子命令一致）：

| 阶段 | 改前 | 改后 | 做法 |
|---|---:|---:|---|
| extract（解压 + 描述符解码，已并行） | ~53 ms | ~53 ms | 前缀解压 + 每 DEX 解码 |
| **sort** | **~47 ms** | **~8.9 ms** | `par_sort_unstable`（比较器是类型的全序，因此与稳定排序结果逐字节相同） |
| dedup | ~1.3 ms | ~1.3 ms | |
| **render（组装 payload）** | **~43 ms** | **~12.8 ms** | 按已排序顺序分块并行渲染，末尾一次拼接 |
| `classes` 墙钟 | 190 ms | **130 ms（−32%）** | 带 `--filter` 时 156 → 96 ms |

16 对配对 A/B：基准包 −60 ms（16/16）、另一个 243 MiB 包 −16 ms（15/16）、`--filter` −60 ms（16/16）；
...
`findrefs` 上 rasc 更费，原因是**用内存换并行度**：每个 worker 持一份在飞的整份 DEX（≤ ~11 MiB），
再加上结果集本身（该查询 170,923 行）与分配器余量。完整权衡曲线（同一查询）：

| `--threads` | 1 | **2** | 4 | 8（默认） | 12 |
|---|---:|---:|---:|---:|---:|
| 峰值 RSS | 262 MiB | **256 MiB** | 315 MiB | 375 MiB | 461 MiB |
| 墙钟 | 1047 ms | **595 ms** | 328 ms | 219 ms | 201 ms |

即：**默认档用 +136 MiB 换 8.9× 速度**；想和参考实现的内存同档（239 MiB）就用 `--threads 2`
—— 仍然比它快 3.3× ✓。

**试过但无效的一刀（记录以免重试）**：在每个条目用完后对其 mmap 区间做
`madvise(MADV_DONTNEED)`（假设峰值里有 ~169 MiB 是可回收的文件页）。实测**无收益**（8 线程
`field INSTANCE`：372.8 → 412.9 MiB，反而偏高且在噪声范围内 ✗）——原因是 macOS 本身就会回收干净的
文件页，而 `ru_maxrss` 是**峰值高水位**，事后丢页不可能降低已经发生过的峰值 ✓。

`classes` 反而是 1 线程高（**453 MiB**），≥2 线程就回到 215–243 MiB——这是 macOS 分配器在单线程下
的高水位特性，不是可回收的内存泄漏 ✓。

**下沉空间有多大？** 峰值 ≈ 结果集（170,923 行 / 567,192 个类，受"排序 + 确定性输出"契约约束，
必须全部驻留 ✓）+ worker 数 × 在飞 DEX（每个 ≤ ~11 MiB ✓）+ 分配器余量。**到这里基本就是地板** ✓：
自适应排序不是瓶颈（53 行的查询只花 0.01 ms，17 万行 7.1 ms，567k 类 8.9 ms —— 成本与结果规模成正比 ✓），
文件页由内核回收 ✓，分配器已回收复用（试过容量预留与 `madvise`，都无可测收益 ✓）。

实测中曾评估、**未采纳**的三个点，供将来判断：
- **外部 `| sort`** 不是替代品：`classes` 58 MB 时 `LC_ALL=C sort` 慢 2.3×（默认 locale 慢 **14.5×** ✗），
  且顺序受运行环境影响 ✗（`LC_ALL=C` 才等于 rasc 的字节序 ✓）；
- **在飞字节预算**（信号量限制同时解压的 DEX 总量）✓ 预计 8 线程下省 ~50–75 MiB、吞吐略降 ✗；
- **流式输出**（`--no-sort` + 按条目序号的有界重排队列 ✓ 顺序仍是确定的 ✓）是唯一的结构性杠杆：
  `classes` 峰值 230 → ~60–90 MiB ✓（省掉结果集，仅留已去重集合）、`findrefs` 375 → ~330 MiB ✓，
  代价是输出顺序改为中央目录序（依赖打包布局 ✗）与 ~100–150 行实现 ✓。默认保持有序输出，不做。
